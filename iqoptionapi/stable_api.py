...
class IQ_Option:
    ...
    def connect(self, sms_code=None):
        ...
        if check == True:
            ...
            try:
                parsed_reason = json.loads(reason) if reason else {}
                if parsed_reason.get('code') == 'verify':
                    response = self.api.send_sms_code(parsed_reason['token'])
    ...
    def __get_digital_open(self):
        """Disabled function as digital options are not supported."""
    ...
    def get_all_open_time(self):
        ...
        binary.join(timeout=30), digital.join(timeout=30), other.join(timeout=30)
    ...
    def get_balance(self):
        # Tenta até 3 vezes obter o saldo
    ...
    def get_balances(self):
        ...
        _timeout = time.time() + 15  # timeout de 15 segundos
        while self.api.balances_raw is None:
            if time.time() > _timeout:
                logging.warning("get_balances timeout (15s)")
                return {"msg": []}
            time.sleep(0.1)
    ...
    def get_candles(self, ACTIVES, interval, count, endtime):
        ...
        while True:
            try:
                ...
                _candle_start = time.time()
                while self.check_connect and self.api.candles.candles_data == None:
                    if time.time() - _candle_start > 30:
                        logging.warning('**warning** get_candles timeout 30s')
                        break
                    time.sleep(0.1)
...
##############################################################################################
    ...
    def sell_option(self, options_ids):
        ...
        while self.api.sold_options_respond == None:
            if time.time() - _t > 30:
                logging.warning('**warning** sell_option timeout 30s')
                return None
            time.sleep(0.1)
    ...
    def sell_digital_option(self, options_ids):
        ...
        while self.api.sold_digital_options_respond == None:
            if time.time() - _t > 30:
                logging.warning('**warning** sell_digital_option timeout 30s')
                return None
            time.sleep(0.1)
...
# __________________for Digital___________________
    ...
    def buy_digital_spot(self, active, amount, action, duration):
        ...
        while self.api.digital_option_placed_id.get(request_id) == None:
            if time.time() - _t > 15:
                logging.warning('**warning** buy_digital_spot timeout 15s')
                return False, None
            time.sleep(0.1)
    ...
    def close_digital_option(self, position_id):
        ...
        while self.get_async_order(position_id)["position-changed"] == {}:
            if time.time() - _t > 60:
                logging.warning('**warning** close_digital_option timeout 60s')
                return None
            time.sleep(0.2)
        ...
        while self.api.result == None:
            if time.time() - _t2 > 30:
                logging.warning('**warning** close_digital_option result timeout 30s')
                return None
            time.sleep(0.1)
    ...
    def check_win_digital_v2(self, buy_order_id):
        ...
        while self.get_async_order(buy_order_id)["position-changed"] == {}:
            if time.time() - _t > 120:
                logging.warning('**warning** check_win_digital_v2 timeout 120s')
                return False, None
            time.sleep(0.5)
    ...
    def get_digital_payout(self, active, seconds=10):
        ...
        start = time.time()
        timeout = seconds if seconds > 0 else 15  # safety timeout de 15s
    ...
    def buy_digital_spot_v2(self, active, amount, action, duration):
        ...
        while self.api.digital_option_placed_id.get(request_id) is None:
            if time.time() - _t > 15:
                logging.warning('**warning** buy_digital_spot_v2 timeout 15s')
                return False, None
            time.sleep(0.1)
