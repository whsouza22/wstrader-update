...
try:
    ...
    try:
        ...
        pass  # Não travar se SSL demorar ou falhar
...
CURRENT_VERSION = "6.9.2" # v6.1.6: previsao visual moderna, sinais persistentes no grafico, timing relaxado
...
def run_bot_from_cli(argv):
    ...
    # ── Carregar .env do usuário (~/.wstrader/.env) ──
    _env_path = os.path.join(os.path.expanduser("~"), ".wstrader", ".env")
    if os.path.isfile(_env_path):
        try:
            from dotenv import load_dotenv
            load_dotenv(dotenv_path=_env_path)
            logger.info("Credenciais carregadas de %s", _env_path)
        except ImportError:
            # Fallback: ler manualmente
            ...
            logger.info("Credenciais carregadas (fallback) de %s", _env_path)
    ...
    _MetaExc = getattr(module, 'MetaAtingidaException', None)
    try:
        module.main()
    except BaseException as _exc:
        if _MetaExc is not None and isinstance(_exc, _MetaExc):
            logger.info("Bot encerrado: meta/stop atingido para %s", broker_key)
            return 0  # Saída normal - meta atingida
        elif isinstance(_exc, (KeyboardInterrupt, SystemExit)):
            raise
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("TelaPrincipal encerrada pelo usuario")
        sys.exit(0)
