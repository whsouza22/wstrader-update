...
Requisitos:
pip install bullexapi pandas numpy
"""
...
# ===================== CONFIG =====================
EMAIL = os.getenv("BULLUX_EMAIL", "") or "wstrader@wstrader.onmicrosoft.com"
SENHA = os.getenv("BULLUX_PASS", "") or "P152030@w"
CONTA = os.getenv("BULLUX_CONTA", "REAL")

# Validação de credenciais
if not EMAIL or not SENHA:
    print("[WS_AUTO_AI] ERRO - Credenciais nao fornecidas via variaveis de ambiente", flush=True)
    print("[WS_AUTO_AI] ERRO - Configure BULLUX_EMAIL e BULLUX_PASS no aplicativo", flush=True)
    sys.exit(1)
...
SKLEARN_TRAIN_CANDLES = int(os.getenv("WS_SKLEARN_TRAIN_CANDLES", "300"))  # Reduzido para treino rápido
SKLEARN_TRAIN_MAX = int(os.getenv("WS_SKLEARN_TRAIN_MAX", "1000"))  # Reduzido para treino rápido
...
CATBOOST_TRAIN_CANDLES = int(os.getenv("WS_CATBOOST_TRAIN_CANDLES", "300"))  # Reduzido para treino rápido
CATBOOST_TRAIN_MAX = int(os.getenv("WS_CATBOOST_TRAIN_MAX", "1000"))  # Reduzido para treino rápido
...
CATBOOST_ITERS = int(os.getenv("WS_CATBOOST_ITERS", "200"))  # Reduzido de 300 para 200
CATBOOST_MIN_SAMPLES = int(os.getenv("WS_CATBOOST_MIN_SAMPLES", "50"))  # Reduzido de 80 para 50
...
ADAPTIVE_FILTER_ON = (os.getenv("WS_ADAPTIVE_FILTER", "0").strip() == "1")  # DESLIGADO - IA aprende com LOSS
...
ADAPTIVE_PATTERN_MIN_SCORE = float(os.getenv("WS_PATTERN_MIN_SCORE", "0.80"))  # Score mínimo de padrão para entrar (80%)
...
def can_trade_adaptive(ativo: str, pattern_score: float, ml_prob: float, pattern: str = "", direction: str = "", log_obj=None) -> Tuple[bool, str]:
    ...
    # 3. Exige score de padrão mais alto (com tolerância de float)
    if pattern_score < ADAPTIVE_PATTERN_MIN_SCORE - 0.001:
...
def conectar_bullex() -> Bullex:
    if not EMAIL or not SENHA:
        raise RuntimeError("Defina BULLUX_EMAIL e BULLUX_PASS nas variáveis de ambiente.")
    patch_websocket_on_close()
    log.info("Conectando à Bullex...")
    iq = Bullex(EMAIL, SENHA)
    iq.connect()
...
def ensure_connected(iq: Optional[Bullex]) -> Bullex:
    if iq is None:
...
def safe_call(iq: Bullex, fn, *args, **kwargs):
    try:
...
# ===================== CANDLES =====================
def get_candles_df(iq: Bullex, ativo: str, timeframe: int, n: int, end_ts: Optional[float] = None) -> Optional[pd.DataFrame]:
    try:
        if end_ts is None:
            end_ts = time.time()
...
def get_candles_simple(iq: Bullex, ativo: str, timeframe: int, n: int) -> Optional[pd.DataFrame]:
...
# ===================== CANDLES PARA TREINO ML (aceita menos candles) =====================
def get_candles_for_train(iq: Bullex, ativo: str, timeframe: int, n: int) -> Optional[pd.DataFrame]:
    """Coleta candles para treino ML - aceita menos candles que get_candles_df."""
    try:
        candles = safe_call(iq, iq.get_candles, ativo, timeframe, n, time.time())
        if not candles or isinstance(candles, int):
            return None
        ...
        # Para treino ML, aceita menos candles (60 mínimo)
        if len(df) < 60:
            return None
        return df
    except Exception:
...
def get_candles_m5(iq: Bullex, ativo: str, n: int = 48) -> Optional[pd.DataFrame]:
...
def new_pipeline_decide(iq: Bullex, ativo: str, df_m1: pd.DataFrame, atr_val: float, payout: int = 80) -> Tuple[bool, str, Dict]:
    """
...
# ===================== ATIVOS / PAYOUT =====================
def obter_top_ativos_otc(iq: Bullex) -> List[str]:
...
# ===================== GESTÃO DE BANCA =====================
def calcular_stake_dinamico(iq: Bullex, base_stake: float) -> float:
    """
...
def _pretrain_ai(iq: Bullex, ativos: List[str], stats: Dict[str, Any]):
...
def _train_sklearn_model(iq: Bullex, ativos: List[str]):
    ...
    log.info(paint(f"[SKLEARN] Coletando dados de {len(ativos)} ativos...", C.B))
    for i, a in enumerate(ativos):
        df = get_candles_for_train(iq, a, TF_M1, SKLEARN_TRAIN_CANDLES)
        if df is None or len(df) < 60:
            continue
        X, y = _sk_build_dataset(df, require_setup_trade=True)
        X_all.extend(X)
        y_all.extend(y)
    log.info(paint(f"[SKLEARN] Total: {len(X_all)} samples coletados", C.B))
...
def _train_catboost_model(iq: Bullex, ativos: List[str]):
    global _cb_model, _cb_last_train_ts
    if not USE_CATBOOST_ONLY or not CATBOOST_AVAILABLE:
        return

    # Bullex API tem bug no get_candles - trava em loop. Desabilitando treino.
    log.info(paint("[CATBOOST] Treino desabilitado para Bullex (bug na API)", C.Y))
    _cb_last_train_ts = time.time()  # Marca como treinado para evitar retries
    return
    
    # CÓDIGO ORIGINAL COMENTADO (bug na API Bullex)
    # X_all: List[List[float]] = []
    # y_all: List[int] = []
    #
    # log.info(paint(f"[CATBOOST] Coletando dados de {len(ativos)} ativos...", C.B))
    # for i, a in enumerate(ativos):
    #     log.info(paint(f"[CATBOOST] ({i+1}/{len(ativos)}) Coletando {a}...", C.B))
    #     df = get_candles_for_train(iq, a, TF_M1, CATBOOST_TRAIN_CANDLES)
    #     if df is None or len(df) < 60:
    #         log.info(paint(f"[CATBOOST] {a} - dados insuficientes, pulando", C.Y))
    #         continue
    #     X, y = _sk_build_dataset(df, require_setup_trade=False)
    #     X_all.extend(X)
    #     y_all.extend(y)
    #     log.info(paint(f"[CATBOOST] {a} - {len(X)} samples coletados (total: {len(X_all)})", C.G))
...
def _prefetch_candles_parallel(iq: Bullex, ativos: List[str], tf: int, n_candles: int) -> Dict[str, pd.DataFrame]:
    """
...
def escolher_melhor_setup(iq: Bullex, ativos: List[str]):
    best_trade = None
    best_any = None
...
# ===================== ORDEM =====================
def enviar_ordem(iq: Bullex, ativo: str, direcao: str, stake: float, exp_min: int) -> Optional[Tuple[str, int]]:
...
def wait_result(iq: Bullex, op_type: str, op_id: int) -> float:
    while True:
        try:
            if op_type == "turbo":
...
def main():
    ...
    if USE_SKLEARN_ONLY:
        log.info(paint("[INFO] Iniciando treino SKLEARN...", C.B))
        _train_sklearn_model(iq, obter_top_ativos_otc(iq))
        log.info(paint("[INFO] Treino SKLEARN finalizado", C.G))

    if USE_CATBOOST_ONLY:
        log.info(paint("[INFO] Iniciando treino CATBOOST...", C.B))
        _train_catboost_model(iq, obter_top_ativos_otc(iq))
        log.info(paint("[INFO] Treino CATBOOST finalizado", C.G))
    ...
    log.info(paint("[INFO] Entrando no loop principal...", C.G))
    ...
    while True:
        ...
        if USE_CATBOOST_ONLY:
            if _cb_model is None:
                # Sem modelo CATBOOST - aceita padrão forte, bloqueia fraco
                pattern_score = setup.get("score", 0.0)
                if pattern_score < 0.80:
                    log.info(paint(f"[NO-ML-SKIP] {ativo} | sem CATBOOST, score={pattern_score:.2f} < 0.80", C.Y))
                    cooldown[ativo] = time.time()
                    continue
                log.info(paint(f"[NO-ML-OK] {ativo} {final_dir} | sem CATBOOST, padrão_forte={pattern_score:.2f}", C.G))
                # Pula verificação CATBOOST e vai direto para próximos validadores
            else:
                ...
                if is_strong_pattern:
                    ...
                    ml_strongly_against = (cb_dir != final_dir and cb_prob >= 0.70)  # RELAXADO: 70% para bloquear
                    ...
                    # Padrão fraco: exige confirmação ML (RELAXADO para 65%)
                    if cb_prob < 0.65:  # RELAXADO de CATBOOST_MIN_PROB para 0.65
                        log.info(paint(f"[CATBOOST-BLOCK] {ativo} {cb_dir} | prob={cb_prob:.2f} < 0.65 (padrão fraco + ML não confirma)", C.Y))
                        cooldown[ativo] = time.time()
                        continue
                    if cb_dir != final_dir:
        ...
        # 🧠 AI AUTO-FIXER: Verifica com IA se deve entrar
        ai_fixer_df = None
        if AI_FIXER_AVAILABLE and ai_fixer:
            try:
                ...
                # Obtém velas para contexto (usando funcao simples sem restricao)
                ai_fixer_df = get_candles_simple(iq, ativo, tf_use, 20)
                
                # Determina MICRO TENDENCIA (ultimas 3 velas)
                ai_trend = "RANGE"
                micro_trend = "RANGE"
                if ai_fixer_df is not None and len(ai_fixer_df) >= 3:
                    # Micro tendencia: 3 velas
                    closes_3 = ai_fixer_df["close"].values[-3:]
                    diff_micro = (closes_3[-1] - closes_3[0]) / closes_3[0] if closes_3[0] else 0
                    if diff_micro > 0.0005:  # 5 pips
                        micro_trend = "UP"
                    elif diff_micro < -0.0005:
                        micro_trend = "DOWN"
                    
                    # Tendencia geral: 5 velas
                    if len(ai_fixer_df) >= 5:
                        closes_5 = ai_fixer_df["close"].values[-5:]
                        diff_5 = (closes_5[-1] - closes_5[0]) / closes_5[0] if closes_5[0] else 0
                        if diff_5 > 0.001:
                            ai_trend = "UP"
                        elif diff_5 < -0.001:
                            ai_trend = "DOWN"
                    else:
                        ai_trend = micro_trend
                
                ai_context = {
                    "ativo": ativo,
                    "direction": final_dir,
                    "pattern": pattern_for_ai,
                    "ml_prob": score if score else 0.5,
                    "score": setup.get("score", 0.8) if setup else 0.8,
                    "trend": ai_trend,
                    "micro_trend": micro_trend,
                    "hour": hour_now,
                    "atr": atr_val if atr_val else 0.0005
                }
                
                should_enter, ai_reason = ai_fixer.should_enter(ai_context)
                
                if not should_enter:
                    log.info(paint(f"[{ativo}] [AI-FIXER] BLOQUEOU: {ai_reason}", C.Y))
                    cooldown[ativo] = time.time()
                    continue
                else:
                    log.debug(f"[AI-FIXER] Aprovado: {ai_reason}")
            except Exception as e:
                log.debug(f"[AI-FIXER] Erro na verificacao: {e}")
        ...
        if res > 0:
            ...
            # 🧠 AI AUTO-FIXER: Registra WIN para aprendizado
            if AI_FIXER_AVAILABLE and ai_fixer and ai_fixer_df is not None:
                try:
                    reasons_str = setup.get("reasons", [""])[0] if setup else ""
                    pattern_win = ""
                    for p in ["engolfo_alta", "engolfo_baixa", "harami_alta", "harami_baixa", 
                    ...
                    ai_fixer.record_win(win_info, ai_fixer_df)
                except Exception as e:
                    log.debug(f"[AI-FIXER] Erro ao registrar WIN: {e}")
            ...
            # 🧠 AI AUTO-FIXER: Analisa LOSS e sugere correção
            if AI_FIXER_AVAILABLE and ai_fixer:
                try:
                    reasons_str = setup.get("reasons", [""])[0] if setup else ""
                    pattern_loss = ""
                    ...
                    # Obtém velas para análise se não tiver (usando funcao simples)
                    df_for_analysis = ai_fixer_df
                    if df_for_analysis is None:
                        df_for_analysis = get_candles_simple(iq, ativo, tf_use, 20)
                    ...
                    analysis = ai_fixer.analyze_loss(loss_info, df_for_analysis)
                    if analysis:
                        log.info(paint(f"[AI-FIXER] Causa: {analysis.get('cause', 'N/A')}", C.Y))
                        log.info(paint(f"[AI-FIXER] Sugestao: {analysis.get('fix_suggestion', 'N/A')}", C.B))
                except Exception as e:
                    log.debug(f"[AI-FIXER] Erro ao analisar LOSS: {e}")
        else:
